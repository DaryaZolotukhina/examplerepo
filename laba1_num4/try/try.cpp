...
int main () {
	...
	cout << "Как вы хотите ввести информацию о графе? \n1 - случайным образом,\n2 - с клавиатуры,\n3 - использовать фиксированный массив. \n0 - выход.\n" << endl;
	int ans;
	cin >> ans;
	while (ans < 0 || ans > 3) //проверка на правильность выбора пункта меню
	{
	...
	if (ans != 0)
	{
		if (ans == 3) //выбран фиксированный массив, для этого графа мин путь из 1 вершины в 5 - 20, макс - 47
		{
			n = 6;
			for (int i = 0; i < n; i++)
				for (int j = 0; j < n; j++)
